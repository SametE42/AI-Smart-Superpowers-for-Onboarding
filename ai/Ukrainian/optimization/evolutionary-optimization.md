...
> Файл джерела: ai/English/optimization/evolutionary-optimization.md
> У разі розбіжностей англійський файл має пріоритет.

Ця сторінка пояснює, як `optimization/evolutionary-optimization.md` використовується в AI Agent Operating Manual. Вона призначена для людей і AI-агентів, які планують, перевіряють або повторюють роботу з репозиторієм.

## Практична область

Використовуйте цю сторінку як операційний орієнтир для теми `optimization`. Вона не замінює докази з репозиторію або інструкції конкретного проєкту.
