...
> Файл джерела: ai/English/models/deepseek/commands.md
> У разі розбіжностей англійський файл має пріоритет.

Ця сторінка пояснює, як `models/deepseek/commands.md` використовується в AI Agent Operating Manual. Вона призначена для людей і AI-агентів, які планують, перевіряють або повторюють роботу з репозиторієм.
