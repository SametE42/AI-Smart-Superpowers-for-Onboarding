...
> Файл джерела: ai/English/prompts/README.md
> У разі розбіжностей англійський файл має пріоритет.

Ця сторінка пояснює, як `prompts/README.md` використовується в AI Agent Operating Manual. Вона призначена для людей і AI-агентів, які планують, перевіряють або повторюють роботу з репозиторієм.

## Практична область

Використовуйте цю сторінку як операційний орієнтир для теми `prompts`. Вона не замінює докази з репозиторію або інструкції конкретного проєкту.
