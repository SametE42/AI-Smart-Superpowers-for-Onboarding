...
> Arquivo de origem: ai/English/agents/researcher-writer-reviewer.md
> Em caso de divergência, o arquivo em inglês continua sendo a fonte autoritativa.

Esta página explica como `agents/researcher-writer-reviewer.md` se encaixa no AI Agent Operating Manual. Ela foi escrita para pessoas e agentes de IA que precisam planejar, verificar ou repetir trabalho no repositório.

## Escopo prático

Use esta página como orientação operacional para o tema `agents`. Ela não substitui evidências do repositório nem instruções específicas do projeto.
