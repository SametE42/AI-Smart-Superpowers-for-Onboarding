...
> Kaynak dosya: ai/English/memory/memory-schema.md
> Tutarsızlık olduğunda İngilizce dosya esas alınır.

Bu sayfa `memory/memory-schema.md` dosyasının AI Agent Operating Manual içinde nasıl kullanıldığını açıklar. Depo çalışmasını planlaması, doğrulaması veya tekrarlaması gereken insanlar ve yapay zeka ajanları için yazılmıştır.
