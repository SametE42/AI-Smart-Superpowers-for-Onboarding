...
> Kaynak dosya: ai/English/tools/claude-code/context-files.md
> Tutarsızlık olduğunda İngilizce dosya esas alınır.

Bu sayfa `tools/claude-code/context-files.md` dosyasının AI Agent Operating Manual içinde nasıl kullanıldığını açıklar. Depo çalışmasını planlaması, doğrulaması veya tekrarlaması gereken insanlar ve yapay zeka ajanları için yazılmıştır.

## Pratik kapsam

Bu sayfayı `tools` konusu için operasyonel yönlendirme olarak kullan. Depo kanıtlarının veya projeye özel talimatların yerine geçmez.
