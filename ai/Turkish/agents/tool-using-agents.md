...
> Kaynak dosya: ai/English/agents/tool-using-agents.md
> Tutarsızlık olduğunda İngilizce dosya esas alınır.

Bu sayfa `agents/tool-using-agents.md` dosyasının AI Agent Operating Manual içinde nasıl kullanıldığını açıklar. Depo çalışmasını planlaması, doğrulaması veya tekrarlaması gereken insanlar ve yapay zeka ajanları için yazılmıştır.
