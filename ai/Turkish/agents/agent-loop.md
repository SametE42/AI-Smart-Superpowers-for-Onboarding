...
> Kaynak dosya: ai/English/agents/agent-loop.md
> Tutarsızlık olduğunda İngilizce dosya esas alınır.

Bu sayfa `agents/agent-loop.md` dosyasının AI Agent Operating Manual içinde nasıl kullanıldığını açıklar. Depo çalışmasını planlaması, doğrulaması veya tekrarlaması gereken insanlar ve yapay zeka ajanları için yazılmıştır.
