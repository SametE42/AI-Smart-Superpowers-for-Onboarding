...
> Fichier source: ai/English/tools/claude-code/context-files.md
> En cas de divergence, le fichier anglais fait autorité.

Cette page explique comment `tools/claude-code/context-files.md` s'inscrit dans l'AI Agent Operating Manual. Elle s'adresse aux personnes et aux agents d'IA qui doivent planifier, vérifier ou répéter un travail de dépôt.

## Portée pratique

Utilise cette page comme repère opérationnel pour le thème `tools`. Elle ne remplace pas les preuves du dépôt ni les consignes propres au projet.
