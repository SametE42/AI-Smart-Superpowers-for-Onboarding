...
> 원본 언어: 영어
> 원본 파일: ai/English/examples/gepa-optimization-example.md
> 차이가 있으면 영어 파일을 기준으로 합니다.

이 페이지는 `examples/gepa-optimization-example.md`가 AI Agent Operating Manual 안에서 어떻게 쓰이는지 설명합니다. 저장소 작업을 계획, 검증 또는 반복해야 하는 사람과 AI 에이전트를 위한 문서입니다.

## 실무 범위

이 페이지를 `examples` 주제의 운영 지침으로 사용하세요. 저장소 증거나 프로젝트 고유 지침을 대체하지 않습니다.
