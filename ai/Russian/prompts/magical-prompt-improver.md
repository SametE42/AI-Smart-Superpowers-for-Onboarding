...
> Исходный файл: ai/English/prompts/magical-prompt-improver.md
> При расхождениях английский файл остается источником истины.

Эта страница объясняет, как `prompts/magical-prompt-improver.md` используется в AI Agent Operating Manual. Она предназначена для людей и AI-агентов, которым нужно планировать, проверять или повторять работу с репозиторием. Source protocol: [`templates/optional/MAGICAL_PROMPT_IMPROVER.md`](../../../templates/optional/MAGICAL_PROMPT_IMPROVER.md).

## Практическая область

Используйте эту страницу как операционное руководство для темы `prompt improvement`. Она не заменяет доказательства из репозитория и проектные инструкции.
