...
> Исходный файл: ai/English/commands/model-specific-notes.md
> При расхождениях английский файл остается источником истины.

Эта страница объясняет, как `commands/model-specific-notes.md` используется в AI Agent Operating Manual. Она предназначена для людей и AI-агентов, которым нужно планировать, проверять или повторять работу с репозиторием.

## Практическая область

Используйте эту страницу как операционное руководство для темы `commands`. Она не заменяет доказательства из репозитория и проектные инструкции.
