...
> Исходный файл: ai/English/workflows/self-evolution-loop.md
> При расхождениях английский файл остается источником истины.

Эта страница объясняет, как `workflows/self-evolution-loop.md` используется в AI Agent Operating Manual. Она предназначена для людей и AI-агентов, которым нужно планировать, проверять или повторять работу с репозиторием.

## Практическая область

Используйте эту страницу как операционное руководство для темы `workflows`. Она не заменяет доказательства из репозитория и проектные инструкции.
