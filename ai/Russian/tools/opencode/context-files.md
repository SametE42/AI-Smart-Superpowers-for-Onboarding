...
> Исходный файл: ai/English/tools/opencode/context-files.md
> При расхождениях английский файл остается источником истины.

Эта страница объясняет, как `tools/opencode/context-files.md` используется в AI Agent Operating Manual. Она предназначена для людей и AI-агентов, которым нужно планировать, проверять или повторять работу с репозиторием.
